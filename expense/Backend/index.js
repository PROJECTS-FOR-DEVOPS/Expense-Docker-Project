...
app.get('/health',(req,res)=>{
    res.json("This is the health check for green-2.0.0");
});
...
app.post('/transaction', (req,res)=>{
    try{
        ...
        transactionService.addTransaction(req.body.amount, req.body.desc, function(err, status){
            if (err) return res.json({ message: 'something went wrong', error: err.message });
            if (status === 200) res.json({ message: 'added transaction successfully' });
        });
    }catch (err){
